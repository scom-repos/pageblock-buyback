define("@buyback/queue-utils",(require, exports)=>{
...
var import_chainlink_sdk = __toModule(require("@openswap/chainlink-sdk"));
...
var getTokenPrice = async (token) => {
  ...
  let chainId = (_a = wallet.chainId) != null ? _a : (0, import_store.getChainId)();
  let tokenPrice;
  let tokenPriceFeedAddress = import_store.ToUSDPriceFeedAddressesMap[chainId][token.toLowerCase()];
  if (tokenPriceFeedAddress) {
    const aggregatorProxy = new import_chainlink_sdk.Contracts.AggregatorProxy(wallet, tokenPriceFeedAddress);
    let tokenLatestRoundData = await aggregatorProxy.latestRoundData();
    let tokenPriceFeedDecimals = await aggregatorProxy.decimals();
    return new import_eth_wallet.BigNumber(tokenLatestRoundData.answer).shiftedBy(-tokenPriceFeedDecimals).toFixed();
  }
  let referencePair = import_store.tokenPriceAMMReference[chainId] && import_store.tokenPriceAMMReference[chainId][token.toLowerCase()];
  if (!referencePair)
    return null;
  const pairContract = new import_sdk.Contracts.OSWAP_Pair(wallet, referencePair);
  let token0 = await pairContract.token0();
  let token1 = await pairContract.token1();
  let reserves = await pairContract.getReserves();
  let token0PriceFeedAddress = import_store.ToUSDPriceFeedAddressesMap[chainId] && import_store.ToUSDPriceFeedAddressesMap[chainId][token0.toLowerCase()];
  let token1PriceFeedAddress = import_store.ToUSDPriceFeedAddressesMap[chainId] && import_store.ToUSDPriceFeedAddressesMap[chainId][token1.toLowerCase()];
  if (token0PriceFeedAddress || token1PriceFeedAddress) {
    if (token0PriceFeedAddress) {
      const aggregatorProxy = new import_chainlink_sdk.Contracts.AggregatorProxy(wallet, token0PriceFeedAddress);
      ...
      let token0USDPrice = new import_eth_wallet.BigNumber(token0LatestRoundData.answer).shiftedBy(-token0PriceFeedDecimals).toFixed();
      if (new import_eth_wallet.BigNumber(token.toLowerCase()).lt(token0.toLowerCase())) {
        tokenPrice = new import_eth_wallet.BigNumber(reserves._reserve1).div(reserves._reserve0).times(token0USDPrice).toFixed();
      } else {
        tokenPrice = new import_eth_wallet.BigNumber(reserves._reserve0).div(reserves._reserve1).times(token0USDPrice).toFixed();
      }
    } else {
      const aggregatorProxy = new import_chainlink_sdk.Contracts.AggregatorProxy(wallet, token1PriceFeedAddress);
      ...
      let token1USDPrice = new import_eth_wallet.BigNumber(token1LatestRoundData.answer).shiftedBy(-token1PriceFeedDecimals).toFixed();
      if (new import_eth_wallet.BigNumber(token.toLowerCase()).lt(token1.toLowerCase())) {
        tokenPrice = new import_eth_wallet.BigNumber(reserves._reserve1).div(reserves._reserve0).times(token1USDPrice).toFixed();
      } else {
        tokenPrice = new import_eth_wallet.BigNumber(reserves._reserve0).div(reserves._reserve1).times(token1USDPrice).toFixed();
      }
    }
  } else {
    if (token0.toLowerCase() == token.toLowerCase()) {
      let token1Price = await getTokenPrice(token1) || "";
      tokenPrice = new import_eth_wallet.BigNumber(token1Price).times(reserves._reserve1).div(reserves._reserve0).toFixed();
    } else {
      let token0Price = await getTokenPrice(token0) || "";
      tokenPrice = new import_eth_wallet.BigNumber(token0Price).times(reserves._reserve0).div(reserves._reserve1).toFixed();
    }
  }
  return tokenPrice;
};
var mapTokenObjectSet = (obj) => {
  ...
  for (let key in obj) {
    if (obj.hasOwnProperty(key)) {
      if (!((_a = obj[key]) == null ? void 0 : _a.address))
        obj[key] = WETH9;
    }
  }
  return obj;
};
var getTokenObjectByAddress = (address) => {
  let chainId = (0, import_store.getChainId)();
...
var getFactoryAddress = (queueType) => {
  switch (queueType) {
...
var getLiquidityProviderAddress = (queueType) => {
  switch (queueType) {
    case import_global.QueueType.PRIORITY_QUEUE:
...
var getPair = async (queueType, tokenA, tokenB) => {
  const wallet = (0, import_store.getWallet)();
  ...
  switch (queueType) {
    case import_global.QueueType.PEGGED_QUEUE:
    case import_global.QueueType.PRIORITY_QUEUE:
      let priorityQ = new import_sdk.Contracts.OSWAP_OracleFactory(wallet, factoryAddress);
      return await priorityQ.getPair(params);
    case import_global.QueueType.RANGE_QUEUE:
      let rangeQ = new import_sdk.Contracts.OSWAP_RangeFactory(wallet, factoryAddress);
...
var getGroupQueueItemsForTrader = async (pairAddress, tokenIn, tokenOut) => {
  ...
  var direction = new import_eth_wallet.BigNumber(tokenIn.address.toLowerCase()).lt(tokenOut.address.toLowerCase());
  ...
  let queueArr = [];
  let tradeFeeObj = getTradeFee(import_global.QueueType.GROUP_QUEUE);
  let tradeFee = new import_eth_wallet.BigNumber(tradeFeeObj.base).minus(tradeFeeObj.fee).div(tradeFeeObj.base).toFixed();
  ...
  for (let i = 0; i < amounts.length; i++) {
    ...
    if (allocation.eq("0"))
      continue;
    let tokenOutAvailable = new import_eth_wallet.BigNumber(amounts[i]).gt(new import_eth_wallet.BigNumber(allocation)) ? allocation : amounts[i];
    let tokenInAvailable = new import_eth_wallet.BigNumber(tokenOutAvailable).dividedBy(new import_eth_wallet.BigNumber(prices[i])).shiftedBy(18 - tokenOut.decimals).dividedBy(new import_eth_wallet.BigNumber(tradeFee)).decimalPlaces(tokenIn.decimals, 1).toFixed();
    queueArr.push({
      pairAddress,
      ...
      tokenInAvailable,
      price: prices[i],
      start: startDates[i].toNumber() * 1e3,
      expire: endDates[i].toNumber() * 1e3,
      allowAll: false,
      locked: locked[i],
      tradeFee
    });
  }
  return queueArr.filter((v) => (0, import_moment.default)().isBetween(v.start, v.expire));
};
var getGroupQueueItemsForAllowAll = async (pairAddress, tokenIn, tokenOut) => {
  let wallet = (0, import_store.getWallet)();
  let chainId = (0, import_store.getChainId)();
  const nativeToken = (0, import_store.getChainNativeToken)(chainId);
  var direction = new import_eth_wallet.BigNumber(tokenIn.address.toLowerCase()).lt(tokenOut.address.toLowerCase());
  const oracleContract = new import_sdk.Contracts.OSWAP_RestrictedPair(wallet, pairAddress);
  ...
  for (let i = 0; i < amounts.length; i++) {
    ...
    let tokenInAvailable = tokenOutAvailable.dividedBy(prices[i]).shiftedBy(18 - tokenOut.decimals).dividedBy(new import_eth_wallet.BigNumber(tradeFee)).decimalPlaces(tokenIn.decimals, 1).toFixed();
    queueArr.push({
      pairAddress,
      tokenIn: isTokenInNative ? nativeToken.symbol : tokenIn.address,
      tokenOut: isTokenOutNative ? nativeToken.symbol : tokenOut.address,
  ...
  return queueArr.filter((v) => (0, import_moment.default)().isBetween(v.start, v.expire) && v.allowAll == true);
};
var getGroupQueueTraderDataObj = async (pairAddress, tokenIn, tokenOut, amountIn, offerIndex) => {
  ...
  queueArr = queueArr.map((v) => {
    return __spreadProps(__spreadValues({}, v), {
      amountIn: new import_eth_wallet.BigNumber(tokenAmountIn).shiftedBy(-tokens.tokenIn.decimals).toFixed(),
      amountOut: new import_eth_wallet.BigNumber(tokenAmountIn).times(v.price).shiftedBy(-18 - Number(tokens.tokenIn.decimals) + Number(tokens.tokenOut.decimals)).times(tradeFee).toFixed()
    });
  }).filter((v) => new import_eth_wallet.BigNumber(v.tokenInAvailable).gte(new import_eth_wallet.BigNumber(v.amountIn))).sort((a, b) => new import_eth_wallet.BigNumber(b.amountOut).minus(a.amountOut).toNumber());
  if (queueArr.length == 0) {
    return {
      sufficientLiquidity: false
    };
  }
  let ratioArr = [(0, import_store.getWallet)().web3.utils.toWei("1")];
  let queueItem;
  if (offerIndex) {
  ...
  let amountOut = queueItem.amount;
  let price = new import_eth_wallet.BigNumber(1).shiftedBy(18).div(queueItem.price).toFixed();
  let priceSwap = new import_eth_wallet.BigNumber(queueItem.price).shiftedBy(-18).toFixed();
  let data = "0x" + (0, import_global.numberToBytes32)((indexArr.length * 2 + 1) * 32) + (0, import_global.numberToBytes32)(indexArr.length) + indexArr.map((e) => (0, import_global.numberToBytes32)(e)).join("") + ratioArr.map((e) => (0, import_global.numberToBytes32)(e)).join("");
  return {
    sufficientLiquidity: true,
    price: parseFloat(price),
    priceSwap: parseFloat(priceSwap),
    amountIn,
    amountOut: new import_eth_wallet.BigNumber(amountOut).shiftedBy(-tokens.tokenOut.decimals).toFixed(),
    data,
    tradeFeeObj
  };
};
...
var getLatestOraclePrice = async (queueType, token, againstToken) => {
  let tokens = mapTokenObjectSet({ token, againstToken });
  let wallet = (0, import_store.getWallet)();
  let address = getFactoryAddress(queueType);
  let factory = new import_sdk.Contracts.OSWAP_OracleFactory(wallet, address);
  let oracleAdapterAddress = await factory.oracles({ param1: tokens.token.address, param2: tokens.againstToken.address });
  let price = "0";
  try {
    const oracleAdaptorContract = new import_oracle_adaptor_sdk.Contracts.OSWAP_OracleChainlink(wallet, oracleAdapterAddress);
...
var getRestrictedPairCustomParams = async () => {
  ...
  const configStoreContract = new import_sdk.Contracts.OSWAP_ConfigStore(wallet, address);
  let feePerOrderRaw = await configStoreContract.customParam(import_eth_wallet.Utils.stringToBytes32(FEE_PER_ORDER).toString());
  let feePerOrder = import_eth_wallet.Utils.fromDecimals(feePerOrderRaw).toString();
  let feePerTraderRaw = await configStoreContract.customParam(import_eth_wallet.Utils.stringToBytes32(FEE_PER_TRADER).toString());
  let feePerTrader = import_eth_wallet.Utils.fromDecimals(feePerTraderRaw).toString();
  let maxDur = await configStoreContract.customParam(import_eth_wallet.Utils.stringToBytes32(MAX_DUR).toString());
...
var getGroupQueuePairInfo = async (pairAddress, tokenAddress, provider, offerIndex) => {
  var _a, _b, _c, _d;
  let wallet = (0, import_store.getWallet)();
  let chainId = (0, import_store.getChainId)();
  const nativeToken = (0, import_store.getChainNativeToken)(chainId);
  const WETH9Address = getAddressByKey("WETH9");
  const _offerIndex = offerIndex ? new import_eth_wallet.BigNumber(offerIndex) : new import_eth_wallet.BigNumber(0);
  ...
  const factoryContract = new import_sdk.Contracts.OSWAP_RestrictedFactory(wallet, factoryAddress);
  const groupPair = new import_sdk.Contracts.OSWAP_RestrictedPair(wallet, pairAddress);
  ...
  let directDirection = !new import_eth_wallet.BigNumber(token0Address.toLowerCase()).lt(token1Address.toLowerCase());
  ...
  let totalAmount = new import_eth_wallet.BigNumber("0");
  let againstToken = token0Address.toLowerCase() == tokenAddress.toLowerCase() ? token1 : token0;
  for (let i = 0; i < amounts.length; i++) {
    if (now >= new Date(expiryDates[i].toNumber() * 1e3).getTime())
      continue;
    totalAmount = totalAmount.plus(amounts[i]);
  }
  let customParams = await getRestrictedPairCustomParams();
  let returnObj = __spreadValues({
    pairAddress: pairAddress.toLowerCase(),
    fromTokenAddress: ((_a = token.address) == null ? void 0 : _a.toLowerCase()) == WETH9Address.toLowerCase() ? nativeToken.symbol : (_b = token.address) == null ? void 0 : _b.toLowerCase(),
    toTokenAddress: ((_c = againstToken.address) == null ? void 0 : _c.toLowerCase()) == WETH9Address.toLowerCase() ? nativeToken.symbol : (_d = againstToken.address) == null ? void 0 : _d.toLowerCase(),
    pairIndex
  }, customParams);
    ...
    const getProviderQueuePairInfo = async function() {
      ...
      for (let j = 0; j < approvedTraderLength; j += 100) {
        ...
        addresses.push(...approvedTrader.trader.map((v, i) => {
          return {
            address: v,
            allocation: new import_eth_wallet.BigNumber(approvedTrader.allocation[i]).shiftedBy(-Number(tokenDecimals)).toFixed()
          };
        }));
      }
      const restrictedPrice = new import_eth_wallet.BigNumber(offer.restrictedPrice).shiftedBy(-18).toFixed();
      return {
        amount: new import_eth_wallet.BigNumber(offer.amount).shiftedBy(-Number(tokenDecimals)).toFixed(),
        reserve: new import_eth_wallet.BigNumber(offer.receiving).shiftedBy(-Number(againstTokenDecimals)).toFixed(),
        startDate: new Date(offer.startDate.toNumber() * 1e3),
        expire: new Date(offer.expire.toNumber() * 1e3),
        locked: offer.locked,
        allowAll: offer.allowAll,
        offerPrice: (0, import_global.toWeiInv)(restrictedPrice).shiftedBy(-18).toFixed(),
        addresses
      };
    };
...
var getGuaranteedBuyBackInfo = async (buybackCampaign) => {
  ...
  if (!info)
    return null;
  if (!info.pairAddress) {
    info.pairAddress = await getPair(import_global.QueueType.GROUP_QUEUE, getTokenObjectByAddress(info.tokenIn), getTokenObjectByAddress(info.tokenOut));
  }
  let queueInfo = await getProviderGroupQueueInfoByIndex(info.pairAddress, info.tokenIn, info.offerIndex);
  let tokenOutPrice = await getTokenPrice(info.tokenOut);
  let tokenInPrice = await getTokenPrice(info.tokenIn);
  let tokenMarketPrice = tokenOutPrice && tokenInPrice ? new import_eth_wallet.BigNumber(tokenOutPrice).dividedBy(tokenInPrice) : new import_eth_wallet.BigNumber("0");
  let one = new import_eth_wallet.BigNumber("1");
  let priceInfo = {
  ...
  let committedAmountInToken = new import_eth_wallet.BigNumber(info.committedAmount).times(queueInfo.offerPrice).toFixed();
  let swappedAmount = new import_eth_wallet.BigNumber(info.committedAmount).minus(queueInfo.allocation).toFixed();
  if (queueInfo.allowAll)
    swappedAmount = "0";
  let lockedAmount = queueInfo.state == "Locked" ? new import_eth_wallet.BigNumber(queueInfo.amount).toFixed() : "0";
  let amountInfo = {
    swappedAmount,
    lockedAmount,
    unlockedAmount: new import_eth_wallet.BigNumber(info.committedAmount).minus(swappedAmount).minus(lockedAmount).toFixed(),
    committedAmount: info.committedAmount,
    committedAmountInToken,
    amountCoverage: new import_eth_wallet.BigNumber(committedAmountInToken).gt(info.idoAmount) ? "1" : new import_eth_wallet.BigNumber(committedAmountInToken).dividedBy(info.idoAmount).toFixed(),
    lockedAmountInPercent: new import_eth_wallet.BigNumber(queueInfo.amount).plus(swappedAmount).dividedBy(info.committedAmount).times(100).toFixed()
  };
  allInfo = __spreadProps(__spreadValues({}, info), {
    queueInfo,
    priceInfo,
    amountInfo
  });
...
var getProviderGroupQueueInfoByIndex = async (pairAddress, tokenInAddress, offerIndex) => {
  let wallet = (0, import_store.getWallet)();
  let chainId = (0, import_store.getChainId)();
  if (!wallet.provider)
  ...
  if (token0Address == tokenInAddress) {
    direction = !new import_eth_wallet.BigNumber(token0Address).lt(token1Address);
  ...
  for (let i = 0; i < approvedTraderLength.toNumber(); i += 100) {
    ...
    addresses.push(...approvedTrader.trader.map((v, i2) => {
      let allo = new import_eth_wallet.BigNumber(approvedTrader.allocation[i2]).shiftedBy(-Number(tokenIn.decimals));
  ...
  let price = (0, import_global.toWeiInv)(new import_eth_wallet.BigNumber(offer.amountAndPrice[1]).shiftedBy(-tokenOut.decimals).toFixed()).shiftedBy(-tokenIn.decimals).toFixed();
  let amount = new import_eth_wallet.BigNumber(offer.amountAndPrice[0]).shiftedBy(-Number(tokenIn.decimals)).toFixed();
  const selectedAddress = wallet.address;
  let available = offer.lockedAndAllowAll[1] ? amount : new import_eth_wallet.BigNumber(await getGroupQueueAllocation(selectedAddress, offerIndex, pairAddress, tokenOut, tokenIn)).shiftedBy(-Number(tokenIn.decimals)).toFixed();
  let tokenInAvailable = new import_eth_wallet.BigNumber(available).dividedBy(new import_eth_wallet.BigNumber(price)).dividedBy(new import_eth_wallet.BigNumber(tradeFee)).toFixed();
  return {
    pairAddress: pairAddress.toLowerCase(),
    ...
    amount,
    offerPrice: price,
    startDate: offer.startDateAndExpire[0].times(1e3).toNumber(),
    endDate: offer.startDateAndExpire[1].times(1e3).toNumber(),
    ...
    willGet: new import_eth_wallet.BigNumber(offer.amountAndPrice[0]).times(new import_eth_wallet.BigNumber(price)).shiftedBy(-Number(tokenIn.decimals)).toFixed(),
    tradeFee,
    tokenInAvailable,
    available
  };
};
var getRangeQueueData = async (pair, tokenA, tokenB, amountOut) => {
  let data = "0x";
  let wallet = (0, import_store.getWallet)();
  let chainId = (0, import_store.getChainId)();
  if (!tokenA.address)
  ...
  let direction = new import_eth_wallet.BigNumber(tokenA.address.toLowerCase()).lt(tokenB.address.toLowerCase());
  let rangePair = new import_sdk.Contracts.OSWAP_RangePair(wallet, pair);
  ...
  if (amounts.length > 1) {
    let index = [];
    let remainingAmt = new import_eth_wallet.BigNumber(amountOut);
    const priceSwap = await getLatestOraclePrice(import_global.QueueType.RANGE_QUEUE, tokenA, tokenB);
    ...
    }).sort((a, b) => new import_eth_wallet.BigNumber(b.amount).minus(a.amount).toNumber());
    for (let i = 0; i < offerArr.length; i++) {
      let offer = offerArr[i];
      let lowerLimit = offer.lowerLimit;
      let upperLimit = offer.upperLimit;
      let expire = offer.expire.toNumber() * 1e3;
      if ((0, import_moment.default)(expire).isBefore((0, import_moment.default)()))
        continue;
      if (new import_eth_wallet.BigNumber(lowerLimit).gt(priceSwap))
        continue;
      if (new import_eth_wallet.BigNumber(upperLimit).gt(0) && new import_eth_wallet.BigNumber(upperLimit).lt(priceSwap))
        continue;
