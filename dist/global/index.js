define("@buyback/global",(require, exports)=>{
...
var formatDate = (date, customType, showTimezone) => {
...
var formatUTCDate = (date, customType, showTimezone) => {
  const formatType = customType || DefaultDateFormat;
  const formatted = (0, import_moment.default)(date).utc().format(formatType);
  return showTimezone ? `${formatted} (UTC)` : formatted;
};
var compareDate = (fromDate, toDate) => {
  if (!toDate) {
    toDate = (0, import_moment.default)();
  }
  return (0, import_moment.default)(fromDate).isSameOrBefore(toDate);
};
var formatNumber = (value, decimals) => {
  let val = value;
  const minValue = "0.0000001";
  if (typeof value === "string") {
    val = new import_eth_wallet2.BigNumber(value).toNumber();
  } else if (typeof value === "object") {
    val = value.toNumber();
  }
  if (val != 0 && new import_eth_wallet2.BigNumber(val).lt(minValue)) {
    return `<${minValue}`;
  }
...
var limitInputNumber = (input, decimals) => {
  ...
  if (!new import_eth_wallet2.BigNumber(amount).isNaN()) {
...
var toWeiInv = (n, unit) => {
  if (new import_eth_wallet2.BigNumber(n).eq(0))
    return new import_eth_wallet2.BigNumber("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
  return new import_eth_wallet2.BigNumber("1").shiftedBy((unit || 18) * 2).idiv(new import_eth_wallet2.BigNumber(n).shiftedBy(unit || 18));
};
...
var numberToBytes32 = (value, prefix) => {
  ...
  if (typeof value == "number") {
    ...
    v = new import_eth_wallet2.BigNumber(value).toString(16);
  } else if (/^(0x)?[0-9A-Fa-f]*$/.test(value)) {
    v = value;
  } else if (import_eth_wallet2.BigNumber.isBigNumber(value)) {
    v = value.toString(16);
  }
  v = v.replace("0x", "");
  v = padLeft(v, 64);
...
async function parseContractError(oMessage, tokens) {
  ...
  return (_a = staticMessageMap[oMessage]) != null ? _a : `Unknown Error: ${oMessage}`;
}
...
var isTransactionConfirmed = async (txHash) => {
  const tx = await import_eth_wallet3.Wallet.getInstance().web3.eth.getTransaction(txHash);
