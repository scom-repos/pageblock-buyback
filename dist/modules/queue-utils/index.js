define("@buyback/queue-utils", ["require", "exports", "@buyback/global", "@ijstech/eth-wallet", "@buyback/store", "@scom/oswap-openswap-contract", "@openswap/chainlink-sdk", "@scom/oswap-oracle-adaptor-contract", "@ijstech/components"], function (require, exports, global_1, eth_wallet_1, store_1, oswap_openswap_contract_1, chainlink_sdk_1, oswap_oracle_adaptor_contract_1, components_1) {
    ...
    const getGuaranteedBuyBackInfo = async (buybackCampaign) => {
        ...
        if (!info)
            return null;
        if (!info.pairAddress) {
            info.pairAddress = await getPair(global_1.QueueType.GROUP_QUEUE, getTokenObjectByAddress(info.tokenIn), getTokenObjectByAddress(info.tokenOut));
        }
        ...
        let tokenMarketPrice = tokenOutPrice && tokenInPrice ? new eth_wallet_1.BigNumber(tokenOutPrice).dividedBy(tokenInPrice) : new eth_wallet_1.BigNumber("0");
        let one = new eth_wallet_1.BigNumber("1");
        let priceInfo = {
        ...
        //Amount Information
        let committedAmountInToken = new eth_wallet_1.BigNumber(info.committedAmount).times(queueInfo.offerPrice).toFixed();
        let swappedAmount = new eth_wallet_1.BigNumber(info.committedAmount).minus(queueInfo.allocation).toFixed();
        if (queueInfo.allowAll)
            swappedAmount = "0"; //Todo: use subgraph
        let lockedAmount = queueInfo.state == 'Locked' ? new eth_wallet_1.BigNumber(queueInfo.amount).toFixed() : "0"; // locked amount + swapped amount
        let amountInfo = {
            swappedAmount,
            lockedAmount,
            unlockedAmount: new eth_wallet_1.BigNumber(info.committedAmount).minus(swappedAmount).minus(lockedAmount).toFixed(),
            committedAmount: info.committedAmount,
            committedAmountInToken,
            amountCoverage: new eth_wallet_1.BigNumber(committedAmountInToken).gt(info.idoAmount) ? "1" : new eth_wallet_1.BigNumber(committedAmountInToken).dividedBy(info.idoAmount).toFixed(),
            lockedAmountInPercent: new eth_wallet_1.BigNumber(queueInfo.amount).plus(swappedAmount).dividedBy(info.committedAmount).times(100).toFixed()
        };
